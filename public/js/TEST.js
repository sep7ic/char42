const firebaseConfig = {
  ...
  let pageId = window.location.pathname.split("/").pop().replace(/\.[^/.]+$/, ""); // Get the unique page ID from the URL
  
  const chatRef = database.ref("comments/" + pageId); // Use the page ID in the reference path for comments
  ...
  function updateDisplayName(displayName) {
    ...
    const uid = user.uid;
    const messagesRef = firebase.database().ref("comments/" + pageId);
    messagesRef.once("value", (snapshot) => {
      snapshot.forEach((childSnapshot) => {
        const messageId = childSnapshot.key;
        const messageData = childSnapshot.val();
        if (messageData.sender === uid) {
          firebase.database().ref("comments/" + pageId + "/" + messageId + "/senderName").set(displayName);
        }
      });
    });
    user.updateProfile({
  ...
  function updateColor(color) {
    ...
    messagesRef.once("value", (snapshot) => {
      snapshot.forEach((childSnapshot) => {
        ...
        const messageData = childSnapshot.val();
        if (messageData.sender === uid) {
          firebase.database().ref("comments/" + pageId + "/" + messageId + "/color").set(color);
        }
      });
    });
    document.getElementById("message").value = "";
